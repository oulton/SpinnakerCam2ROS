        ...
        for (unsigned int imageCnt = 0; imageCnt < k_numImages; imageCnt++)
        {
            try
            {
                ...
                else
                {
                    ...
                    unsigned int rowBytes = (double)pResultImage->GetImageSize() / (double)height;
                    cv::Mat frame = cv::Mat(height, width, CV_8UC3, pResultImage->GetData(), rowBytes);//将图像转换为Mat格式
                    ...
                    cout << "GetBufferSize: " << convertedImage->GetBufferSize() << endl;

                    // void* pData = convertedImage->GetData();
